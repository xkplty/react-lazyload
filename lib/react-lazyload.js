...
var add = wrap('addEventListener', 'attachEvent');
var remove = wrap('removeEventListener', 'detachEvent');
...
var LazyLoad = /** @class */ (function (_super) {
    ...
    function LazyLoad(props) {
        ...
        _this.getEventNode = function () {
            if (_this.props.scrollContainer) {
                ...
                return scrollDom || getScrollParent(reactDom.findDOMNode(_this));
            }
            return getScrollParent(reactDom.findDOMNode(_this));
        };
        ...
        _this.lazyLoadHandler = function () {
            ...
            if (isInViewport(node, eventNode, offset)) {
        ...
        _this.detachListeners = function () {
            var eventNode = _this.getEventNode();
            remove(window, 'resize', _this.lazyLoadHandler);
            remove(eventNode, 'scroll', _this.lazyLoadHandler);
        };
        _this.state = { visible: false };
        if (props.throttle > 0) {
            if (props.debounce) {
                _this.lazyLoadHandler = lodash_debounce(_this.lazyLoadHandler, props.throttle);
            }
            else {
                _this.lazyLoadHandler = lodash_throttle(_this.lazyLoadHandler, props.throttle);
            }
        }
        return _this;
    }
    ...
    LazyLoad.prototype.render = function () {
        ...
        var elClasses = ('LazyLoad' +
            (visible ? ' is-visible' : '') +
            (className ? " " + className : ''));
        return React__default.createElement(this.props.elementType, {
            className: elClasses,
            style: elStyles,
        }, visible ? React.Children.only(children) : fallbackUI);
    };
